...
function migrate(source){
  ...
  return {schemaVersion:SCHEMA_VERSION,categories,products,needs,promotions,combos,deliveryZones,delivery,selection,banner,settings:{...defaults.settings,...(source.settings||{})}};
}
...
window.HGStore={STORAGE_KEY,SCHEMA_VERSION,defaults:clone(defaults),clone,valid,dataIsValid:value=>valid(migrate(value)),migrate,load,save:persist,getCategories,getProducts,saveCategories,saveProducts,uid,slugify,uniqueSlug,stringList,getInventoryStatus,findCategory,findProduct,findNeed,whatsappUrl,reset:()=>persist(clone(defaults))};
})();
